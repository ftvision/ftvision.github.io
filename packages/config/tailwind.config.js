/**
 * Shared Tailwind CSS configuration with design token integration
 *
 * This preset provides:
 * - Semantic color system (ground, figure, border, action, status, etc.)
 * - Typography scale with font tokens
 * - Spacing system (stack, inline, inset, gutter, section)
 * - Motion tokens (durations, easing)
 * - Custom plugins for typography, spacing, transitions, and accessibility
 *
 * Usage in app tailwind.config.js:
 *   const sharedConfig = require('@blog/config/tailwind.config.js');
 *   module.exports = {
...
/**
 * Typography utility plugin - provides .type-* classes
 */
...
/**
 * Spacing utility plugin - provides semantic spacing classes
 */
...
/**
 * Motion transition presets plugin
 */
const motionPlugin = plugin(function({ addUtilities }) {
  addUtilities({
 ...
 */
const accessibilityPlugin = plugin(function({ addUtilities, addBase }) {
  addUtilities({
    '.sr-only': {
      position: 'absolute',
      width: '1px',
