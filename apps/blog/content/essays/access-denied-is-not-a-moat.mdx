...
This incident is not exactly the same as Anthropic's own commercial access decisions. The immediate order came from the government, not from Anthropic's product team. But it sits inside a larger pattern that Anthropic itself has helped make respectable: the belief that frontier AI advantage can be protected by controlling access.

To be clear, Anthropic does not literally say "access denial is a moat." The stronger and fairer target is the strategic premise underneath its public argument: that controlling access to models, chips, and the American AI stack can preserve a lead long enough to create safety, leverage, and better terms of engagement. That is the logic this essay is attacking.
...
/>. It is fair to say that, for a while, Claude Code was the coding-agent product to beat. If you believe frontier AI is a short-window national-security race, and if you believe your lead is one of the few things preventing a worse world, then access control becomes tempting.
...
/>. This does not prove that a cutoff produced a particular rival model. It shows the simpler strategic point: once access becomes uncertain, replacing the dependency becomes rational.

<XEmbed tweetId="2009686466746822731" className="my-8" />

Still, the strategic lesson is obvious enough. If a rival is casually using your model because it is convenient, the dependency is your advantage. If you cut them off, the dependency becomes their emergency. Before, the rival could postpone the internal replacement. After, the replacement has an owner, a deadline, and a reason to receive resources.

OpenAI's modern Codex makes the point. Codex returned in 2025 as a cloud-based software engineering agent, then evolved into GPT-5-Codex and later GPT-5.3-Codex, a model OpenAI described as the most capable agentic coding model it had released to date<Reference
...
/>.<Note>Huang's point is about dependency as the practical form of U.S. leadership. By 2026, he had led Nvidia for 33 years since founding it in 1993, after earlier roles at <a href="https://nvidianews.nvidia.com/bios/jensen-huang" target="_blank" rel="noopener noreferrer">LSI Logic and AMD</a>. He understands that leadership lives not only in chip shipments, but in CUDA, libraries, developer habits, vendor relationships, and the decision to stay on the American stack instead of building around it.</Note>
...
The DeepSeek-Huawei story shows both sides of this. Earlier reporting said DeepSeek's attempt to train R2 on Huawei Ascend chips ran into stability, interconnect, and software-stack problems, forcing it back to Nvidia for training while using Ascend for inference. That is real friction. But newer reports say a Huawei-linked research group completed full-parameter post-training of DeepSeek's 1.6-trillion-parameter V4-Pro on more than 1,000 Ascend 910C chips, and that DeepSeek V4-Pro was built around Ascend from the outset<Reference
...
This does not prove export controls failed. That would be too simple. Dario Amodei's counterargument is that DeepSeek does not show controls are useless; it shows that controls need to be stronger and better enforced, especially at the scale of millions of chips<Reference
...
/>. Arena reported that GLM-5.2 Max ranked number two in Code Arena: Frontend, behind only Fable 5 and ahead of Claude Opus 4.7 Thinking. Then Z.ai founder Jie Tang replied to Elon Musk's estimate of a Chinese Fable-class model by saying it "won't take that long."
...
First, the follower's job is easier than the pioneer's. The pioneer must prove that a path exists. The follower gets to run down a path whose existence has already been demonstrated. Once the market knows that coding agents matter, every serious lab can justify spending on coding agents. Once the market knows that reasoning traces excite users, every serious lab can test the same surface. Once the market knows that smaller, cheaper, good-enough open models can win adoption, every serious open-weight developer has a target.
